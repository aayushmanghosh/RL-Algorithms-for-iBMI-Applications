function MPA = getMPA(trial1,trial2,K)
% getMPA Computing the Minimum Principal Angles
%
%    getMPA helps us in calculating the Minimum Principal Angles, which is
%    a pasrsimonous scalar metric used to evaluate the level of similarity
%    between different neural datasets.
%
%    # Inputs:
%      -'trial1': Any session (trial for a specific day) from the expt.
%      -'trial2': Any session (trial for a specific day) from the expt.
%      - 'K': Denotes the number of classes.
%
%    # Outputs:
%      -'MPA': Calculating the MPA between tuples.
%
%    # References:
...
%               function is to calculate the minimum principal angles between
%               experimental data recorded on diff. days in an experiment.
%               Lower values of MPA corresponds to higher value of
%               similarity between recorded data. The original idea is to
%               calculate the mean of MPAs to get a metric of variability
%               exhibited by neural activity.
%
...
% Computing the zscore of each trials.
trial1 = zscore(trial1);
trial2 = zscore(trial2);

% Computing the covariance matrix of each trials.
cov1 = cov(trial1);
cov2 = cov(trial2);

% Performing singular value decomposition of the Covariance matrices
[U1,~,~] = svd(cov1);
[U2,~,~] = svd(cov2);

% Calculating the minimum principal angles between two different subspaces.
Thetas = subspacea(U1(:,1:K),U2(:,1:K));
MPA = min(Thetas); % Calculating the Minimum Principal Angles.
end
